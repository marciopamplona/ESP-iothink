...
boolean CPlugin_008(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case CPLUGIN_PROTOCOL_TEMPLATE:
      {
        event->String1 = "";
        event->String2 = F("demo.php?name=%devicename%&task=%sensortag%&valuename=%measuretag%&value=%value%");
        break;
      }
...
boolean HTTPSend(struct EventStruct *event, byte varIndex, float value, unsigned long longValue)
{
  ...
  url.replace(F("%measuretag%"), URLEncode(ExtraTaskSettings.TaskDeviceValueNames[varIndex]));
  if (longValue)
